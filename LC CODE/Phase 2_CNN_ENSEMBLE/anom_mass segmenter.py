...
def get_trn_valid_files(model_type, valid, trn_percentage=80, frame_count=8):
    print("Get trn/valid files.")
    file_paths = glob.glob("resources/segmenter_trndata/" + "*_1.png")
    file_paths.sort()
    trn_res = []
    valid_res = []
    for index, file_path in enumerate(file_paths):
        ...
        if "1.3.6.1.4" in file_name or "spie" in file_name or "TIME" in file_name:
            ...
            if operators.get_patient_fold(patient_id) % 3 != valid:
                trn_set = True

        if trn_set:
            trn_res.append((file_path, overlay_path))
        else:
            valid_res.append((file_path, overlay_path))
...
class DumpPredictions(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        generator = image_generator(self.dump_filelist, 1, trn_set=False, model_type=self.model_type)
        for i in range(0, 10):
            x, y = next(generator)
...
def image_generator(batch_files, batch_size, trn_set, model_type):
    global ELASTIC_INDICES
    while True:
        if trn_set:
        ...
        for batch_file_idx, batch_file in enumerate(batch_files):
            ...
            if trn_set:
                if random.randint(0, 100) > 50:
                    for img_index, img in enumerate(imgs):
                        imgs[img_index] = elastic_transform(img, 128, 15)
                    overlay = elastic_transform(overlay, 128, 15)

                if True:
                    augmented = imgs + [overlay]
            ...
            imgs3d = numpy.vstack(imgs)
            imgs3d = imgs3d.swapaxes(0, 3)

            img_list.append(imgs3d)
...
#UNET FROM GRT123
def get_unet(l_r, load_weights_path=None) -> Model:
    inputs = Input((variables.SEGMENTER_IMG_SIZE, variables.SEGMENTER_IMG_SIZE, 1))
    filter_size = 32
    growth_step = 32
    ...
    model.compile(optimizer=SGD(lr=l_r, momentum=0.9, nesterov=True), loss=dice_coef_loss, metrics=[dice_coef])

    model.summary()
    return model


def trn_model_unet(valid, model_type, continue_from=None):
    batch_size = 4
    trn_percentage = 80 if model_type == "masses" else 90
    trn_files, valid_files = get_trn_valid_files( model_type, valid, trn_percentage, frame_count=1)
    tmp_gen = image_generator(trn_files[:2], 2, True, model_type)
    for i in range(10):
        x = next(tmp_gen)
        img = x[0][0].reshape((variables.SEGMENTER_IMG_SIZE, variables.SEGMENTER_IMG_SIZE))
        img *= 255
        img = x[1][0].reshape((variables.SEGMENTER_IMG_SIZE, variables.SEGMENTER_IMG_SIZE))
        img *= 255

    trn_gen = image_generator(trn_files, batch_size, True, model_type)
    valid_gen = image_generator(valid_files, batch_size, False, model_type)
    ...
    checkpoint1 = ModelCheckpoint("workdir/" + model_type +"_model_h" + str(valid) + "_{epoch:02d}-{val_loss:.2f}.hd5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
    checkpoint2 = ModelCheckpoint("workdir/" + model_type +"_model_h" + str(valid) + "_best.hd5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
    files = []
    idx = 0
    while (idx < (len(valid_files))):
        files.append(valid_files[idx])
        idx += 5
    dumper = DumpPredictions(valid_files[::10], model_type)
    epoch_div = 1
    epoch_count = 200 if model_type == "masses" else 50
    model.fit_generator(trn_gen, len(trn_files) / epoch_div, epoch_count, validation_data=valid_gen, nb_val_samples=len(valid_files) / epoch_div, callbacks=[checkpoint1, checkpoint2, dumper])
    if not os.path.exists("models"):
        os.mkdir("models")
    shutil.copy("workdir/" + model_type +"_model_h" + str(valid) + "_best.hd5", "models/" + model_type +"_model_h" + str(valid) + "_best.hd5")

def predict_patients(patients_dir, model_path, valid, patient_predictions, model_type):
    model = get_unet(0.001)
    ...
    for item_name in os.listdir(patients_dir):
        ...
        if valid >= 0:
            patient_fold = operators.get_patient_fold(patient_id, submission_set_neg=True)
            if patient_fold < 0:
                if valid != 0:
                    continue
            else:
                patient_fold %= 3
                if patient_fold != valid:
                    continue
        ...
        for img_path in slices:
            ...
            src_img = cv2.resize(src_img, dsize=(variables.SEGMENTER_IMG_SIZE, variables.SEGMENTER_IMG_SIZE))
            src_img = prepare_image_for_net(src_img)
            p = model.predict(src_img, batch_size=1)
            p[p < 0.5] = 0
            mass += p.sum()
            p = p[0, :, :, 0] * 255
            src_img = src_img.reshape((variables.SEGMENTER_IMG_SIZE, variables.SEGMENTER_IMG_SIZE))
            src_img *= 255
        ...
        df.to_csv(variables.BASE_DIR + model_type + "_predictions.csv", index=False)


def pipeline_main():
    continue_from = None
    for model_type_name in ["masses"]:
        trn_model_unet(valid=0, model_type=model_type_name, continue_from=continue_from)
        trn_model_unet(valid=1, model_type=model_type_name, continue_from=continue_from)
        trn_model_unet(valid=2, model_type=model_type_name, continue_from=continue_from)

    for model_type_name in ["masses"]:
        patient_predictions_global = []
        for valid_no in [0, 1, 2]:
            patient_base_dir = variables.NDSB3_EXTRACTED_IMAGE_DIR
            predict_patients(patients_dir=patient_base_dir, model_path="models/" + model_type_name + "_model_h" + str(valid_no) + "_best.hd5", valid=valid_no, patient_predictions=patient_predictions_global, model_type=model_type_name)
